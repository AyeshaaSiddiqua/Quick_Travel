...
def edit_desti(request,pk):
    ...
    if request.method == "POST":
        if len(request.FILES) != 0:
            if len(d.img1) > 0:
               os.remove(d.img1.path)
            d.img1 = request.FILES['image1']
            os.path.join(d.img1.path)

            if len(d.img2) > 0:
               os.remove(d.img2.path)
            d.img2 = request.FILES['image2']
            os.path.join(d.img2.path)
...
def edit_pacakage(request,dest_name):
    ...
    if request.method == "POST":
        if len(request.FILES) != 0:
            if len(p.img1) > 0:
               os.remove(p.img1.path)
            p.img1 = request.FILES['pack_image1']
            os.path.join(p.img1.path)

            if len(p.img2) > 0:
               os.remove(p.img2.path)
            p.img2 = request.FILES['pack_image2']
            os.path.join(p.img2.path)
